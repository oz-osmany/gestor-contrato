...
passport.use('local.signin', new LocalStrategy({
    ...
    if (rows.length > 0) {
        ...
        if (validPassword) {
    const filas=await pool.query('SELECT * FROM `db_distribucion` INNER JOIN db_usuarios on db_distribucion.id_company=db_usuarios.id_company WHERE db_usuarios.id=1');
           if (filas[0]){
